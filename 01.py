...
with open('c:/temp/ips.txt', 'r') as arquivo:  # Abrir arquivo
    for linha in arquivo:
        # ler as linhas com strip para limpar as linhas vazias
        ip = linha.strip()
...
with open('c:/temp/ips_saida.txt', 'w') as arquivo:  # Escrevendo arquivo
    arquivo.writelines('[Endereços válidos]\n')

    for ip in ips_validos:
        arquivo.writelines(f'{ip}\n')

    arquivo.writelines('\n')  # Pulando duas linhas com \n
    arquivo.writelines('\n')
    arquivo.writelines('[Endereços válidos]\n')
